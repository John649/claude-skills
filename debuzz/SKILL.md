...
2. Run agy in headless mode. **agy's headless mode does not read stdin**, so instruct it to read the input file by absolute path, composing the prompt from the shared style rules plus the chosen mode's audience instructions. For example, colleague mode:
   ...
   Give the command a generous timeout (120s+ — pass `--print-timeout 5m` if runs get cut off); the CLI can be slow to first token. If the default model is too slow, `--model gemini-3.5-flash-medium` is a faster choice for a pure rewrite task.
